...
function SettingsView() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
        <h3 className="text-sm font-black text-slate-900 mb-4 flex items-center gap-2">
          <Settings className="w-4 h-4 text-slate-400" /> System Control
        </h3>
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
          <div>
            <p className="text-xs font-black text-slate-900 uppercase tracking-tight">Manual Withdrawal Window</p>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Force open window for all users</p>
          </div>
          <button 
            type="button"
            onClick={() => setSettings({...settings, manualWithdrawUnlock: !settings.manualWithdrawUnlock})}
            className={`w-12 h-6 rounded-full relative transition-colors duration-300 ${settings.manualWithdrawUnlock ? 'bg-emerald-500' : 'bg-slate-300'}`}
          >
            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-300 ${settings.manualWithdrawUnlock ? 'left-7' : 'left-1'}`}></div>
          </button>
        </div>
      </div>
